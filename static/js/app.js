(() => {
  ...
  // ── API docs: downloadable integration guide (.md) ──────────────────────────
...
\`\`\`bash
curl -X POST ${baseUrl}/api/cleanup \\
  -H "X-API-Key: ${apiKey}" \\
  -H "Content-Type: application/json" \\
...
---
*File này được tạo tự động từ AutoDown (${baseUrl}) — tải lại nếu API key thay đổi.*
`;
  }

  document.getElementById("download-guide-btn")?.addEventListener("click", async () => {
    await _apiKeyReady;
    const md = buildApiGuideMarkdown(window.location.origin, _apiKey || "<API_KEY_CUA_BAN>");
    const blob = new Blob([md], { type: "text/markdown" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a");
    a.href = url;
    a.download = "AutoDown-API-Guide.md";
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
